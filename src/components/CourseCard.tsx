  ...
  return (
    <div className=" grid h-40 w-full grid-cols-12 bg-white shadow-xl transition-shadow hover:shadow-lg">
      <div className="col-span-3 overflow-hidden">
        <img className="h-40 w-full" src={course.pictrue} alt={course.title} />
      </div>
      ...
      <div className="col-span-2 flex flex-col gap-4 self-center pr-4">
        <Link to={course.id} className="btn btn-primary capitalize">
          Continue <HiArrowRight className="ml-4 text-xl" />
        </Link>
        <a
          target={"_blank"}
          ...
          // onClick={() => {
          //   if (!course.liveLink) toast("No live session is scheduled now.");
          // }}
          rel="noreferrer"
        >
